...
def upgrade():
    ...
    conn.execute(
        text(
            "ALTER TABLE account CHANGE default_calendar_id emailed_events_calendar_id INTEGER"
        )
    )
    conn.execute(
        text(
            "ALTER TABLE account ADD CONSTRAINT emailed_events_fk "
...
def downgrade():
    ...
    conn.execute(
        text(
            "ALTER TABLE account ADD CONSTRAINT account_ibfk_10 "
