...
def test_api_pessimistic_update(db, api_client, calendar, default_account):
    ...
    e_resp = api_client.post_data(
        "/events", e_data, headers={"Api-Version": API_VERSIONS[1]}
    )
...
def test_api_pessimistic_delete(db, api_client, calendar, default_account):
    ...
    e_delete_resp = api_client.delete(
        "/events/" + e_id, headers={"Api-Version": API_VERSIONS[1]}
    )
    assert e_delete_resp.status_code == 200
...
def test_api_filter(db, api_client, calendar, default_namespace):
    ...
    events = api_client.get_data("/events?description=\u2713")
    assert len(events) == 1
