...
def test_naked_domain():
    ...
    assert naked_domain("iplayer.forums.bbc.co.uk") == "iplayer.forums.bbc.co.uk"
    assert naked_domain("parliament.org.au") == "parliament.org.au"
    assert (
        naked_domain("prime-minister.parliament.org.au")
        == "prime-minister.parliament.org.au"
    )
    assert (
        naked_domain("https://python.linux.com/resume-guido.pdf") == "python.linux.com"
    )
    assert naked_domain("ftp://linux.com/vmlinuz") == "linux.com"
    assert naked_domain("ftp://parliament.co.uk/vmlinuz") == "parliament.co.uk"
    assert naked_domain("ftp://pm.parliament.co.uk/vmlinuz") == "pm.parliament.co.uk"
    assert (
        naked_domain("https://username:password@python.linux.com/vmlinuz")
        == "python.linux.com"
    )
...
def test_extract_emails_from_text():
    ...
    assert extract_emails_from_text('<a href="mailto:test@example.com">Email</a>') == [
        "test@example.com"
    ]
