...
def upgrade():
    # Thread table
    op.create_index('ix_namespace_id__cleaned_subject', 'thread', ['namespace_id', '_cleaned_subject'], unique=False, mysql_length={'_cleaned_subject': 80})
    ...
    op.create_index('ix_message_subject', 'message', ['subject'], unique=False, mysql_length=80)

    conn = op.get_bind()
    conn.execute('ALTER TABLE `message` CHANGE `data_sha256` `data_sha256` VARCHAR(64)  CHARACTER SET ascii  NULL  DEFAULT NULL')
...
def downgrade():
    ...
    op.create_index('ix_cleaned_subject', 'thread', ['_cleaned_subject'], unique=False, mysql_length={'_cleaned_subject': 191})
    op.drop_index('ix_namespace_id__cleaned_subject', table_name='thread')

    op.drop_index('ix_thread_subject', 'thread')
    op.create_index('ix_thread_subject', 'thread', ['subject'], unique=False, mysql_length=191)
    ...
    conn = op.get_bind()
    conn.execute('ALTER TABLE `message` CHANGE `data_sha256` `data_sha256` VARCHAR(255)  NULL  DEFAULT NULL;')
    op.create_index('ix_message_data_sha256', 'message', ['data_sha256'], unique=False, mysql_length=191)
