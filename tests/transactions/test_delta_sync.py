...
def add_account_with_different_namespace_id(
    ...
    account = GenericAccount(
        ...
        desired_sync_host=platform.node(),
        provider="yahoo",
    )
...
def get_cursor(api_client, timestamp):
    cursor_response = api_client.post_data(
        "/delta/generate_cursor", {"start": timestamp}
    )
...
def test_account_delta_for_different_namespace_id(db):
    ...
    txns, _ = format_transactions_after_pointer(
        namespace, 0, db.session, 10, exclude_account=True
    )
    assert not txns

    # Verify `account` delta is returned when exclude_account=False
    txns, _ = format_transactions_after_pointer(
        namespace, 0, db.session, 10, exclude_account=False
    )
    assert txns
