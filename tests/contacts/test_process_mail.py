...
def test_update_contacts_from_message(db, default_namespace, thread):
    ...
    add_fake_message(
        ...
        from_addr=[("", "alpha@example.com")],
        cc_addr=[("", "alpha@example.com")],
        to_addr=[("", "beta@example.com"), ("", "gamma@example.com")],
    )
    ...
    assert (
        ...
        .filter(
            ...
            Contact.namespace_id == default_namespace.id,
        )
        .count()
        == 3
    )
...
def test_handle_noreply_addresses(db, default_namespace, thread):
    ...
    noreply_contact = (
        ...
        .filter(
            ...
            Contact.email_address == "drive-shares-noreply@google.com",
        )
        .one()
    )
    ...
    contact = (
        ...
        .filter(
            Contact.namespace == default_namespace,
            Contact.email_address == "alice@example.com",
        )
        .first()
    )
