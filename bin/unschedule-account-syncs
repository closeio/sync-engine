...
def main(dry_run, number, hostname, process):
    ...
    if not dry_run:
        message = 'It is unsafe to unassign hosts while mailsync processes are running. '\
                  'Have you shut down the appropriate mailsync processes on {}? [Y/n]'.format(hostname)
        if raw_input(message).strip().lower() == 'n':
            print 'Bailing out'
            return
