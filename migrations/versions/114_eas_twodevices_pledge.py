...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("eas_device_id", sa.String(length=32), nullable=False),
    ...
    op.create_index(
        "ix_easdevice_created_at", "easdevice", ["created_at"], unique=False
    )
    op.create_index(
    ...
    op.add_column(
        ...
        sa.Column(
            "device_id",
            sa.Integer(),
            sa.ForeignKey("easdevice.id", ondelete="CASCADE"),
            nullable=True,
        ),
    )
    op.drop_constraint("account_id", "easfoldersyncstatus", type_="unique")
    op.create_unique_constraint(
        None, "easfoldersyncstatus", ["account_id", "device_id", "folder_id"]
    )
    op.drop_constraint(
        "easfoldersyncstatus_ibfk_1", "easfoldersyncstatus", type_="foreignkey"
    )
    op.drop_constraint(
        "easfoldersyncstatus_ibfk_2", "easfoldersyncstatus", type_="foreignkey"
    )
