...
from flanker.addresslib.address import (  # type: ignore[import-untyped]
    MAX_ADDRESS_LENGTH,
)
from flanker.addresslib.quote import (  # type: ignore[import-untyped]
    smart_quote,
)
from flanker.mime.message.headers import (  # type: ignore[import-untyped]
    WithParams,
)
...
def fallback_to_base64(  # type: ignore[no-untyped-def]  # noqa: ANN201
    charset, preferred_encoding, body
):
...
def create_email(  # type: ignore[no-untyped-def]  # noqa: ANN201, D417
    from_name,
    from_email,
    reply_to,
    nylas_uid,
...
def encode_string(value, maxlinelen):  # type: ignore[no-untyped-def]  # noqa: ANN201
    try:
...
def _get_full_spec_without_validation(  # type: ignore[no-untyped-def]
    name, email
):
    """
...
def add_nylas_headers(msg, nylas_uid) -> None:  # type: ignore[no-untyped-def]
    """
...
def generate_message_id_header(uid) -> str:  # type: ignore[no-untyped-def]
    return f"<{uid}@mailer.nylas.com>"


def _rfc_transform(msg):  # type: ignore[no-untyped-def]
    """
