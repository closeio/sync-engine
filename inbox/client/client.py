...
from .util import generate_id, url_concat
...
def _validate(response):
    ...
    if DEBUG:
        print(
            "{} {} ({}) => {}: {}".format(
                request.method, url, data, status_code, response.text
            )
        )
    ...
    if status_code == 200:
        ...
        raise APIClientError(
            url=url, status_code=status_code, data=data, message="Unknown status code."
        )
...
class APIClient(json.JSONEncoder):
    ...
    def access_token(self, value):
        ...
        if value:
            self.session.headers.update({"Authorization": "Bearer " + value})
        else:
    ...
    def _get_resources(self, cls, extra=None, **filters):
        ...
        return list(
            filter(
                lambda x: x is not None, map(lambda x: cls.create(self, **x), results)
            )
        )

    @nylas_excepted
    def _get_resource_raw(self, cls, id, extra=None, headers=None, **filters):
    ...
    def _call_resource_method(self, cls, id, method_name, data):
        ...
        if cls.api_root != "a":
            ...
            # Management method.
            url = "{}/a/{}/{}/{}/{}".format(
                self.api_server, self.app_id, cls.collection_name, id, method_name
            )
