...
def sorted_gmail_messages(db, default_account, sorted_gmail_threads, folder):
    ...
    message1 = add_fake_message(
        ...
        thread=thread1,
        g_msgid=1,
        from_addr=[{"name": "Ben Bitdiddle", "email": "ben@bitdiddle.com"}],
        to_addr=[{"name": "Barrack Obama", "email": "barrack@obama.com"}],
...
def sorted_imap_messages(
    ...
    message1 = add_fake_message(
        ...
        thread=thread1,
        from_addr=[{"name": "", "email": "inboxapptest@example.com"}],
        to_addr=[{"name": "Ben Bitdiddle", "email": "ben@bitdiddle.com"}],
        received_date=datetime.datetime(2015, 7, 9, 23, 50, 7),
        subject="YOO!",
    )
...
@fixture
def different_imap_messages(
    db, generic_account, different_sorted_imap_threads, different_imap_folder
):
    ...
    add_fake_imapuid(
        db.session, generic_account.id, message1, different_imap_folder, 5000
    )
    ...
    add_fake_imapuid(
        db.session, generic_account.id, message2, different_imap_folder, 5001
    )
    ...
    add_fake_imapuid(
        db.session, generic_account.id, message3, different_imap_folder, 5002
    )
...
@pytest.mark.parametrize("is_streaming", [True, False])
def test_invalid_gmail_account_search(
    db,
    api_client,
    default_account,
    invalid_gmail_token,
...
@pytest.mark.parametrize("is_streaming", [True, False])
def test_invalid_imap_account_search(
    db,
    imap_api_client,
    generic_account,
