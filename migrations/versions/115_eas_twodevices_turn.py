...
def upgrade():
    ...
    class EASAccount(Base):
        ...
        primary_device = sa.orm.relationship(
            "EASDevice",
            primaryjoin="and_(EASAccount.primary_device_id == EASDevice.id, "
            "EASDevice.deleted_at.is_(None))",
            uselist=False,
        )
        secondary_device = sa.orm.relationship(
            "EASDevice",
            primaryjoin="and_(EASAccount.secondary_device_id == EASDevice.id, "
            "EASDevice.deleted_at.is_(None))",
            uselist=False,
        )
    ...
    acct_device_map = dict(
        (id_, device_id)
        for id_, device_id in conn.execute(
            text("""SELECT id, secondary_device_id from easaccount""")
        )
    )
