...
class GmailAuthHandler(OAuthAuthHandler):
    ...
    def _authenticate_IMAP_connection(self, account, conn):
        ...
        try:
            ...
            token = g_token_manager.get_token_for_email(account, force_refresh=True)
            try:
                conn.oauth2_login(account.email_address, token)
            except IMAPClient.Error as exc:
                exc = _process_imap_exception(exc)
    ...
    def update_account(self, account, response):
        ...
        if new_refresh_token:
            ...
            if (
                len(account.valid_auth_credentials) == 0
        ...
        account.sync_contacts = account.sync_contacts or response.get("contacts", True)
    ...
    def validate_token(self, access_token):
        response = requests.get(
            self.OAUTH_TOKEN_VALIDATION_URL, params={"access_token": access_token}
        )
