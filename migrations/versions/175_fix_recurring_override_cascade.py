...
def upgrade() -> None:
    ...
    if fk_delete == "CASCADE":
        print(
            f"Checked fk: {fk_name}. This migration is not needed, skipping."
        )
        return
    ...
    conn.execute(
        text(
            "ALTER TABLE recurringeventoverride DROP FOREIGN KEY "
            "`recurringeventoverride_ibfk_2`"
        )
    )
    conn.execute(
        text(
            "ALTER TABLE recurringeventoverride ADD CONSTRAINT recurringeventoverride_ibfk_2"
