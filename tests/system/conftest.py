...
try:
    ...
    gmail_accounts = [
        NylasTestClient(email, API_BASE)
        for email, password in credentials
        if "gmail.com" in email or "inboxapp.com" in email
    ]
...
def timeout_loop(name):
    def wrap(f):
        def wrapped_f(*args, **kwargs):
            ...
            assert success, "Failed to {} in less than {}s on {}".format(
                name, TEST_MAX_DURATION_SECS, client.email_address
            )

            format_test_result(name, client.provider, client.email_address, start_time)
            return True
