...
class Message(NylasAPIObject):
    ...
    def labels(self):
        if self._labels:
            return [Label.create(self.api, **label) for label in self._labels]
        else:
            return []
...
class Account(NylasAPIObject):
    ...
    def upgrade(self):
        self.api._call_resource_method(self, self.account_id, "upgrade", None)

    def downgrade(self):
        self.api._call_resource_method(self, self.account_id, "downgrade", None)
