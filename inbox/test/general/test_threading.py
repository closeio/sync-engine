...
from inbox.test.util.base import add_fake_message, add_fake_thread, add_fake_imapuid
...
def test_message_cleanup():
    ...
    assert (
        cleanup_subject("Undeliverable: Message returned to sender")
        == "Message returned to sender"
    )
    assert (
        cleanup_subject("Undelivered: Message returned to sender")
        == "Message returned to sender"
    )
...
def test_basic_message_grouping(db, default_namespace):
    ...
    msg2 = add_fake_message(
        ...
        from_addr=[("Some random dude", "random@pobox.com")],
